...
def draw_rules():
    pr.draw_text("FIND THE HEX", 50, 200, 80, pr.RAYWHITE)
    pr.draw_text(
        "FIND THE [R]ED, [G]REEN, [B]LUE COMPONENTS", 100, 300, 20, pr.RAYWHITE
    )
    pr.draw_text("OF THE HEX COLOR", 260, 320, 20, pr.RAYWHITE)

    pr.draw_text("ONCE A COMPONENT IS CHOSEN", 180, 350, 20, pr.RAYWHITE)
    pr.draw_text("THERE'S NO COMING BACK", 220, 370, 20, pr.RAYWHITE)

    pr.draw_text("'GENERATE COLOR' TO START", 200, 400, 20, pr.RAYWHITE)
    pr.draw_text("GOOD LUCK", 260, 460, 40, pr.RAYWHITE)
...
async def main():
    ...
    while not pr.window_should_close():
        ...
        if red_container:
            red_container.draw(show_tip=show_tip)
        if green_container:
            green_container.draw(show_tip=show_tip)
        if blue_container:
            blue_container.draw(show_tip=show_tip)
        ...
        if red_container and green_container and blue_container:
            if (
                ...
                if (
                    ...
                    show_tip_rect = pr.Rectangle(540, 670, 120, 20)
                    pr.draw_rectangle_rec(show_tip_rect, pr.RED)
                    pr.draw_text("show hints", 550, 670, 20, pr.BLACK)

                    if pr.check_collision_point_rec(
                        pr.get_mouse_position(), show_tip_rect
                    ):
                        pr.draw_rectangle_rec(show_tip_rect, pr.Color(55, 0, 0, 255))
                        pr.draw_text("show hints", 550, 670, 20, pr.BLACK)
                        if pr.is_mouse_button_pressed(0):
                            show_tip = True
